...
export const FullAnimationStudio: React.FC<FullAnimationStudioProps> = ({
  ...
  useEffect(() => {
    ...
    posedBones.filter((bone) => bone.visible !== false).forEach((bone) => {
      ...
      const color = new THREE.Color(selected ? '#ffffff' : bone.color || '#00b4c4');
    ...
    for (let i = 0; i <= count; i++) {
      ...
      ticks.push(
        <div key={i} style={{ left: `${pct}%` }} className="absolute top-0 bottom-0 flex flex-col items-center pointer-events-none">
          ...
          {isMajor && <span className="text-[9px] font-mono text-[#00b4c4] mt-0.5">{timeVal.toFixed(1)}s</span>}
        </div>
      );
    }
    return ticks;
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#16191e] text-[#eaedf1] font-sans select-none overflow-hidden">
      {/* Upper Area: 3D Viewport + Left Clip Inspector */}
      <div className="flex-1 flex overflow-hidden border-b border-[#3a3f4a] relative">
        <div className="w-60 bg-[#1c1f26] border-r border-[#3a3f4a] flex flex-col z-10">
          <div className="h-8 bg-[#1c1f26] border-b border-[#3a3f4a] px-3 flex items-center justify-between font-mono text-[10px] text-[#00b4c4] font-bold">
            <span className="flex items-center gap-1.5 uppercase">
              <Film className="w-3.5 h-3.5 text-[#00b4c4]" />
              CLIPS LIBRARY
            </span>
            <button
              onClick={() => {
              ...
              className="px-2 py-0.5 cad-button text-[#00b4c4] text-[9px] font-bold"
            >
              + Clip
            </button>
          </div>
                ...
                return (
                  <div
                    ...
                    className={`p-2 rounded border flex items-center justify-between cursor-pointer font-mono text-xs transition ${
                      isActive
                        ? 'bg-[rgba(20,115,230,0.22)] border-[#00b4c4] text-[#00b4c4] font-bold shadow-md shadow-none'
                        : 'bg-[#16191e] border-[#3a3f4a] text-[#a6abb4] hover:border-[#3a3f4a]'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <Activity className={`w-3.5 h-3.5 ${isActive ? 'text-[#00b4c4]' : 'text-[#858a93]'}`} />
                      <span>{clip.name}</span>
                    </div>
                    <span className="text-[10px] text-[#858a93] font-mono">{clip.duration}s</span>
                  </div>
                );
              })}
            </div>
        ...
        <div ref={containerRef} className="flex-1 h-full relative bg-[#111318]">
          <div className="absolute top-2 left-2 flex items-center gap-2 pointer-events-none z-10 font-mono text-[10px]">
            <span className="cad-card px-2.5 py-1 text-[#00b4c4] font-bold border-[#3a3f4a] uppercase flex items-center gap-1">
              <Sparkles className="w-3 h-3 text-[#00b4c4]" />
              ANIMATION STUDIO VIEWPORT
            </span>
            <span className="cad-card px-2.5 py-1 text-[#00b4c4]">
      ...
      <div className="h-72 bg-[#0a0d14] border-t border-[#3a3f4a] flex flex-col select-none">
        {/* Top Transport & Action Toolbar */}
        <div className="h-10 bg-[#1c1f26] border-b border-[#3a3f4a] px-4 flex items-center justify-between font-mono text-xs">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setIsPlaying(!isPlaying)}
              className={`p-1.5 rounded-full shadow-lg transition flex items-center justify-center ${
                isPlaying ? 'bg-[#00b4c4] text-white shadow-none' : 'bg-[#00b4c4] text-white shadow-none'
              }`}
            >
            ...
            <button
              ...
              className="p-1.5 cad-button text-[#858a93] hover:text-white"
              title="Rewind to 0.0s"
            >
              <RotateCcw className="w-3.5 h-3.5" />
            </button>

            <div className="font-mono text-xs font-bold text-[#00b4c4] bg-[#16191e] px-3 py-1 rounded border border-[#3a3f4a]">
              {currentTime.toFixed(2)}s / {activeClip.duration.toFixed(2)}s
            </div>

            {/* Dope Sheet vs Graph Curve View Switcher */}
            <div className="flex items-center gap-1 bg-[#16191e] p-0.5 rounded border border-[#3a3f4a]">
              <button
                onClick={() => setTimelineMode('dopesheet')}
                className={`px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 ${
                  timelineMode === 'dopesheet' ? 'bg-[#00b4c4] text-white shadow-sm' : 'text-[#858a93] hover:text-white'
                }`}
              >
              ...
              <button
                ...
                className={`px-2 py-0.5 rounded text-[10px] font-bold flex items-center gap-1 ${
                  timelineMode === 'graph' ? 'bg-[#00b4c4] text-white shadow-sm' : 'text-[#858a93] hover:text-white'
                }`}
              >
            ...
            <div className="flex items-center gap-1 bg-[#16191e] px-2 py-0.5 rounded border border-[#3a3f4a]">
              <span className="text-[9px] text-[#858a93] uppercase">CURVE:</span>
              <button
                onClick={() => setEasingCurve('smooth')}
                className={`px-1.5 py-0.5 rounded text-[9px] ${
                  easingCurve === 'smooth' ? 'bg-[#00b4c4] text-white font-bold' : 'text-[#858a93]'
                }`}
              >
                SMOOTH
              </button>
              <button
                ...
                className={`px-1.5 py-0.5 rounded text-[9px] ${
                  easingCurve === 'bounce' ? 'bg-[#00b4c4] text-white font-bold' : 'text-[#858a93]'
                }`}
              >
                BOUNCE
              </button>
          ...
          <div className="flex items-center gap-2">
            ...
            {unTrackedMeshes.length > 0 && (
              <button
                ...
                className="px-2 py-1 cad-button text-[#00b4c4] font-bold border-[#3a3f4a] text-[10px] flex items-center gap-1"
                title="Add Track for Mesh"
              >
                <Plus className="w-3 h-3 text-[#00b4c4]" />
                <span>+ TRACK</span>
              </button>
            )}
            ...
            <button
              ...
              className="px-2 py-1 cad-button text-[#00b4c4] font-bold border-[#3a3f4a] text-[10px] flex items-center gap-1"
            >
              <Move className="w-3 h-3 text-[#00b4c4]" />
              <span>+ POS</span>
            </button>
            <button
              onClick={() => handleAddChannelKeyframe(selectedMeshId, 'rot')}
              className="px-2 py-1 cad-button text-[#ec5b62] font-bold border-[#3a3f4a] text-[10px] flex items-center gap-1"
            >
            ...
            <button
              ...
              className="px-2 py-1 cad-button text-[#00b4c4] font-bold border-[#3a3f4a] text-[10px] flex items-center gap-1"
            >
              <Maximize2 className="w-3 h-3 text-[#00b4c4]" />
              <span>+ SCL</span>
            </button>
            <button
              onClick={() => handleAddChannelKeyframe(selectedMeshId, 'all')}
              className="px-2.5 py-1 bg-[#00b4c4] text-white font-bold rounded shadow-md shadow-none text-[10px] flex items-center gap-1"
            >
            ...
            <div className="relative">
              <button
                ...
                className="px-2 py-1 cad-button text-emerald-400 font-bold border-[#3a3f4a] text-[10px] flex items-center gap-1"
              ...
              {uvClipMenuOpen && (
                <div className="absolute right-0 top-full mt-1 z-50 w-48 bg-[#1a1c22] border border-[#3a3f4a] rounded shadow-2xl p-1.5 text-xs font-mono">
                  <div className="text-[9px] uppercase tracking-wide text-[#858a93] px-1 py-0.5 border-b border-[#1a1c22] mb-1 font-bold">
                  ...
                  {(() => {
                    ...
                    return clipsList.map((clip) => (
                      <button
                        ...
                        className="w-full text-left px-2 py-1 rounded hover:bg-[#00b4c4]/20 hover:text-[#00b4c4] flex items-center justify-between text-[11px] font-semibold text-[#d8d8d8]"
                      >
            ...
            <button
              ...
              className="px-2 py-1 cad-button text-cyan-400 font-bold border-[#3a3f4a] text-[10px] flex items-center gap-1"
        ...
        <div className="flex border-b border-[#3a3f4a] bg-[#21242c]">
          <div className="w-56 px-3 py-1 border-r border-[#3a3f4a] font-mono text-[10px] text-[#858a93] uppercase font-bold flex items-center gap-1.5">
        ...
        {timelineMode === 'graph' ? (
          ...
          <div className="flex-1 relative bg-[#06090f] p-4 flex items-center justify-center">
            <svg className="w-full h-full overflow-visible">
              {/* Grid Lines */}
              <line x1="0" y1="50%" x2="100%" y2="50%" stroke="#282c35" strokeDasharray="4 4" />
              ...
              {activeClip.tracks
                ...
                .map((t) => {
                  ...
                  return (
                    <g key={t.targetId}>
                      <path d={posPath} fill="none" stroke="#00b4c4" strokeWidth="2.5" />
                    </g>
                  );
                })}
            </svg>
            <div className="absolute top-2 right-2 bg-black/60 px-2 py-1 rounded text-[10px] font-mono text-[#00b4c4] border border-[#3a3f4a]">
              BEZIER CURVE GRAPH VIEW
            </div>
          </div>
        ) : (
          /* Standard Multi-Track Dope Sheet View */
          <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#282c35]">
                ...
                <div key={track.targetId} className="border-b border-[#182333]">
                  ...
                  <div
                    ...
                    className={`flex h-8 items-center transition cursor-pointer font-mono text-xs ${
                      isSelected ? 'bg-[rgba(237,115,0,0.18)]/40 border-l-2 border-[#00b4c4]' : 'bg-[#0f1622] hover:bg-[#151f2e]'
                    }`}
                  >
                    <div className="w-56 px-3 flex items-center justify-between border-r border-[#3a3f4a]">
                      <div
                      ...
                      >
                        {isExpanded ? (
                          <ChevronDown className="w-3.5 h-3.5 text-[#00b4c4]" />
                        ) : (
                          <ChevronRight className="w-3.5 h-3.5 text-[#858a93]" />
                        )}
                        {track.targetType === 'bone' && <Bone className="w-3 h-3 text-[#00b4c4]"/>}
                        <span className={isSelected ? 'text-[#00b4c4]' : 'text-[#eaedf1]'}>{track.targetName}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className="p-1 text-[#858a93] hover:text-[#ec5b62]"
                        ...
                        <button
                          ...
                          className="p-1 text-[#858a93] hover:text-[#00b4c4]"
                    ...
                    <div className="flex-1 relative h-full flex items-center px-2">
                      <div className="absolute inset-x-0 h-px bg-[#1a1c22]" />
                      ...
                      {Array.from(
                        ...
                        return (
                          <div
                            ...
                            className="absolute transform -translate-x-1/2 w-3.5 h-3.5 bg-[#00b4c4] rotate-45 border-2 border-[#1a1c22] cursor-pointer hover:bg-[#00b4c4] transition z-20 shadow-md shadow-none"
                          />
                        );
                      })}
                    </div>
                  ...
                  {isExpanded && (
                    <div className="bg-[#282c35] space-y-px">
                      {/* Position Sub-Track */}
                      <div className="flex h-6 items-center font-mono text-[10px]">
                        <div className="w-56 pl-8 pr-3 flex items-center gap-1.5 text-[#00b4c4] font-bold border-r border-[#3a3f4a]">
                          <Move className="w-3 h-3 text-[#00b4c4]" />
                          ...
                          {track.posKeyframes.map((kf) => {
                            ...
                            return (
                              <div
                                ...
                                className={`absolute transform -translate-x-1/2 w-3 h-3 bg-[#00b4c4] rotate-45 border border-[#1a1c22] cursor-pointer transition z-20 ${
                                  isKfSelected ? 'ring-2 ring-white bg-white scale-125 z-30' : 'hover:bg-[#00b4c4]'
                                }`}
                      ...
                      {/* Rotation Sub-Track */}
                      <div className="flex h-6 items-center font-mono text-[10px]">
                        <div className="w-56 pl-8 pr-3 flex items-center gap-1.5 text-[#ec5b62] font-bold border-r border-[#3a3f4a]">
                          <RotateCw className="w-3 h-3 text-[#ec5b62]" />
                          ...
                          {track.rotKeyframes.map((kf) => {
                            ...
                            return (
                              <div
                                ...
                                className={`absolute transform -translate-x-1/2 w-3 h-3 bg-[#00b4c4] rotate-45 border border-[#1a1c22] cursor-pointer transition z-20 ${
                                  isKfSelected ? 'ring-2 ring-white bg-[#282c35] scale-125 z-30' : 'hover:bg-[#00b4c4]'
                                }`}
                      ...
                      {track.targetType === 'mesh' && (
                        <div className="flex h-6 items-center font-mono text-[10px]">
                          <div className="w-56 pl-8 pr-3 flex items-center gap-1.5 text-emerald-400 font-bold border-r border-[#3a3f4a]">
                            <Clapperboard className="w-3 h-3 text-emerald-400" />
                            ...
                            {(track.textureClipKeys || []).map((ck) => {
                              ...
                              return (
                                <div
                                  ...
                                  className="absolute transform -translate-x-1/2 px-1.5 py-0.5 bg-emerald-600 text-white rounded text-[8px] font-bold cursor-pointer hover:bg-emerald-500 transition z-20 shadow-md border border-[#1a1c22] flex items-center gap-1"
                      ...
                      {track.targetType === 'mesh' && track.texFrameKeyframes && track.texFrameKeyframes.length > 0 && (
                        <div className="flex h-6 items-center font-mono text-[10px]">
                          <div className="w-56 pl-8 pr-3 flex items-center gap-1.5 text-cyan-400 font-bold border-r border-[#3a3f4a]">
                            <ImageIcon className="w-3 h-3 text-cyan-400" />
                            ...
                            {track.texFrameKeyframes.map((kf) => {
                              ...
                              return (
                                <div
                                  ...
                                  className="absolute transform -translate-x-1/2 px-1 py-0.2 bg-cyan-600 text-white rounded text-[8px] font-mono font-bold cursor-pointer hover:bg-cyan-500 transition z-20 border border-[#1a1c22]"
