...
export const ParticleStudioModal: React.FC<ParticleStudioModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 font-sans">
      <div className="w-full max-w-5xl h-[80vh] bg-[#1c1c1c] border border-[#3e3e3e] rounded-lg shadow-2xl flex flex-col overflow-hidden">
        <div className="h-10 px-3 border-b border-[#323232] flex items-center justify-between bg-[#141414]">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              className="cad-button h-7 px-2 text-[#2680eb] text-[10px] font-bold flex items-center gap-1"
            ...
            <button type="button" className="p-1.5 hover:bg-[#323232] rounded" onClick={onClose}>
        ...
        <div className="flex-1 flex min-h-0">
          <div className="w-72 border-r border-[#323232] overflow-y-auto p-3 space-y-3 text-[10px] font-mono custom-scrollbar">
              ...
              <div className="grid grid-cols-2 gap-1">
                {PARTICLE_PRESETS.map((preset) => (
                  <button
                    ...
                    type="button"
                    title={preset.hint}
                    className="h-7 rounded border border-[#323232] hover:border-[#e68619] text-[9px] font-bold"
                    onClick={() => {
            ...
            <label className="block space-y-1">
              ...
              <input type="range" min={0.1} max={5} step={0.05} value={draft.lifetime} onChange={(e) => patch({ lifetime: Number(e.target.value) })} className="w-full accent-[#1473e6]" />
            </label>
            <label className="block space-y-1">
              <span className="text-[#8c8c8c] uppercase">Speed ({draft.startSpeed.toFixed(2)})</span>
              <input type="range" min={0} max={8} step={0.1} value={draft.startSpeed} onChange={(e) => patch({ startSpeed: Number(e.target.value) })} className="w-full accent-[#2680eb]" />
            </label>
            <label className="block space-y-1">
          ...
          <div className="flex-1 relative bg-[#0e1218]">
            ...
            <div className="absolute bottom-3 left-3 text-[9px] font-mono text-[#8c8c8c] bg-[#121212]/80 px-2 py-1 rounded border border-[#323232]">
